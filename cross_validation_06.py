...
def compute_branch_accuracy(v_xs, v_ys):
    ...
    y_pre1 = sess.run(branch_prediction, feed_dict={xs: v_xs, keep_prob: 1})
    correct_prediction = tf.equal(tf.argmax(y_pre1,1), tf.argmax(v_ys,1))
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
    result = sess.run(accuracy, feed_dict={xs: v_xs, ys: v_ys, keep_prob: 1})
    return result
...
def compute_all_branch_accuracy(v_xs, v_ys):
    ...
    data_size = len(v_xs)
    for i in range(data_size):  
       y_pre1 = sess.run(branch_prediction, feed_dict={xs: v_xs[np.newaxis,i], keep_prob: 1})
       if np.argmax(y_pre1,1) == np.argmax(v_ys[i]):
           sum_correct += 1
...
#branch
branch_flat = tf.reshape(pool1,[-1,16*16*64])
...
joint_loss = 0.4*loss+0.6*branch_loss
...
for train_index, test_index in kf.split(x_train_):
    ...
    for i in range(epochs):
        ...
        #branch
        acc = compute_all_branch_accuracy(
            x_train, y_train)
        branch_acc.append(acc)
        loss3 = sess.run(branch_loss,feed_dict = {xs:x_train[:1000], ys: y_train[:1000],keep_prob: 1})
        ...
        acc = compute_all_branch_accuracy(
            x_val, y_val)
        loss4 = sess.run(branch_loss,feed_dict = {xs:x_val[0:1000,:,:], ys: y_val[0:1000,:],keep_prob: 1})
...
#    plt.savefig('branch_loss2.png')
    ...
    name = '%s_%d_%s' % ('./joint_06_1116_',count,'/model.ckpt')
