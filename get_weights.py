...
weights = tf.get_default_graph().get_tensor_by_name(
  os.path.split(local_conv1_1.name)[0] + '/kernel:0')

bias = tf.get_default_graph().get_tensor_by_name( os.path.split(local_conv1_1.name)[0] + '/bias:0')
...
weights = tf.get_default_graph().get_tensor_by_name(
  os.path.split(branch_logits.name)[0] + '/kernel:0')


branch_weights = weights.eval(session=sess)

bias = tf.get_default_graph().get_tensor_by_name( os.path.split(branch_logits.name)[0] + '/bias:0')

branch_bias = bias.eval(session = sess)
"""
